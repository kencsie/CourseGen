...
- **先列證據，再給分。** 你必須在 reason 中同時列出優點（✅）和問題（❌），然後根據兩者的比例和嚴重程度決定分數。如果你無法列出任何具體的 ✅ 優點和 ❌ 問題，說明你的審查不夠仔細，請重新檢查。
...
- accuracy (事實準確性，基於 sources 驗證):
  ...
  3=多數 claim 有 sources 支撐，但有 1-2 處明顯措辭誇大、過度推論、或在 sources 中完全找不到依據
  4=所有核心 claim 都有 sources 支撐，但有 1-2 處改述略偏離 source 原文（如簡化後語意微偏、或合理推論但 source 未明確說明）
  5=逐條查核後，所有事實性 claim 都能在 sources snippet 中找到直接對應依據；常識性說明不需要 source 支撐
  ⚠️ accuracy 只評估「內容是否與 sources 一致」，不評估「是否有加引用標記」。如果某 claim 的內容與 sources 吻合但漏了 [N] 標記，accuracy 不扣分（由 citation 維度處理）。
- completeness (完整性):
  1=大量遺漏關鍵知識
